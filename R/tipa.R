...
#' (in the same units as `y`). If the RMS errors pre-stimulus and post-stimulus are
...
#'
#' @export
tipaCosinor = function(time, y, stimOnset, stimDuration=0, periodGuess=24, trend=TRUE, shortcut=TRUE) {
	...
	if (shortcut) {
		...
		# find expected time of first fit peak after stimOnset (ok even if non-zero transients)
